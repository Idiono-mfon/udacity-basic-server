...
(async () => {
  ...
  app.get("/cars", async (req: Request, res: Response) => {
    ...
    if (make) {
      const cars: Car[] = cars_list.filter((car) => car.make === make);
  ...
  app.get("/cars/:id", async (req: Request, res: Response) => {
    ...
    const car: Car | undefined = cars_list.find((car) => car.id === +id);

    if (!car) return res.status(404).send("Car does not exist");
  ...
  app.post("/cars", async (req: Request, res: Response) => {
    ...
    return res.status(201).json(new_car);
  });
